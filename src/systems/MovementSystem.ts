...
export class MovementSystem extends IteratingSystem {
	constructor() { super(Family.all(PositionComponent, VelocityComponent).get()); }
	...
	protected processEntity(entity: Entity, deltaTime: number): void {
		...
		if (pc.position.x < 0)
			pc.position.x += 800;
		else if (pc.position.x >= 800)
			pc.position.x -= 800;
		if (pc.position.y < 0)
			pc.position.y += 600;
		else if (pc.position.y >= 600)
			pc.position.y -= 600;

		// calculate all 4 other (portal-ish) positions
		let xFactor = pc.position.x < 400 ? 1 : -1;
		let yFactor = pc.position.y < 300 ? 1 : -1;
		let x2 = pc.position.x + 800 * xFactor;
		let y2 = pc.position.y + 600 * yFactor;
