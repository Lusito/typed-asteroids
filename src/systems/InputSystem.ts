...
export class InputSystem extends IteratingSystem {
    ...
    public onKeyDown(e: KeyboardEvent) {
        if (e.keyCode === Key.Space && this.gameEvents && !this.keys[Key.Space])
            this.gameEvents.shoot.emit();
        if (e.keyCode < this.keys.length) this.keys[e.keyCode] = true;
    }

    public onKeyUp(e: KeyboardEvent) {
        if (e.keyCode < this.keys.length)
    ...
    protected processEntity(entity: Entity, deltaTime: number): void {
        ...
        if (!pc || !vc || !plc || !this.gameEvents)
            return;
        let accelScale = 0, turnScale = 0;
        ...
        if (turnScale)
            pc.rotation += (turnScale * 200.0) * deltaTime;
        if (accelScale) {
